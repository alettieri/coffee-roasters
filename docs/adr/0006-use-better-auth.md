...
Public Discovery is unauthenticated, while Journals and Visits require a signed-in Local Coffee Lover. The private admin section additionally requires explicit maintainer authorization.
...
- enforce Journal and Visit ownership in server-side application operations;
- enforce maintainer authorization on every administrative operation, not only through route visibility;
...
- Authorization remains an application responsibility; authentication alone does not grant access to private Journal or admin data.
