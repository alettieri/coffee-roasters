...
The application uses Docker PostgreSQL locally, Neon when deployed, and Cloudflare Workers for staging and production. The prior decisions did not specify the deployed connection path, database roles, or whether preview infrastructure was shared or created per pull request.
...
- **staging**: one shared Cloudflare Worker with an isolated Neon staging branch; and
- **production**: Cloudflare Worker with the Neon production branch or project.

Do not create per-pull-request Cloudflare or Neon environments until parallel development creates a measured need.

Deployed Worker traffic will connect to Neon through a separately configured Cloudflare Hyperdrive binding for staging and production. Drizzle migrations will use a direct Neon connection outside the Worker runtime.
...
- define all Wrangler environment bindings explicitly because environment bindings are not inherited;
...
- Hyperdrive provides the production pooling and Worker-to-PostgreSQL connection boundary.
